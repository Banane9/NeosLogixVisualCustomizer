...
{
    internal static class EnumDropdown
    {
        private static readonly MethodInfo buildFlagUI = typeof(EnumDropdown).GetMethod("BuildFlagUi", BindingFlags.Static | BindingFlags.NonPublic); // Store this for later :)

        public static void AddDropdownBtn(UIBuilder ui, IField target)
        {
            var btn = ui.Button("▼");
